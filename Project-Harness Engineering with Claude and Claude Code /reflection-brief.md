...
19. **What broke.** One thing that failed first try in your environment, and how you fixed it. (If nothing, what you checked to be sure.)
    → System 1's first live run crashed instantly with `TypeError: Client.__init__() got an unexpected keyword argument 'proxies'`. Root cause: `anthropic==0.39.0` passes a `proxies` kwarg that the freshly-installed `httpx 0.28.1` removed. I fixed it by pinning `httpx<0.28` (resolved to `0.27.2`) in that venv, after which the run completed. I also had to `export ANTHROPIC_BASE_URL=https://claude.vocareum.com` — without it the `voc-` key is sent to `api.anthropic.com` and 401s. Separately, haiku left 4/8 claims `incomplete` (`summary.md`); switching to `claude-sonnet-4-6` raised it to 4 routed but still left 4 incomplete, confirming that is genuine model behavior (the model ends its turn in prose instead of calling a terminal tool), not a harness bug — the 29 tests including the anti-pattern suite still pass.

20. **What you'd change.** One architectural decision you'd make differently, grounded in what you observed.
    → The harness lets the model reach `end_turn` without ever calling a terminal tool, which shows up as `incomplete` outcomes for `claim_01`, `claim_03`, `claim_06`, and `claim_07` in `evidence/system-1-claims/summary.md` — including `claim_06`, which was *expected to escalate*. I'd add a harness-level completion guard: on `end_turn` when `session.terminal_called` is False, inject a one-line system reminder ("you must call route_to_adjuster or escalate_to_human before finishing") and re-prompt once, then fall back to an automatic escalation if it still declines. That keeps control flow deterministic and `stop_reason`-driven while guaranteeing every claim reaches a terminal state, turning 4/8 incompletes into routed-or-escalated.
